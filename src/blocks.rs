...
pub unsafe fn bit_width_unchecked(a: *const Limb, n: usize) -> usize {
	if n == 0 {
		...
		unsafe {
			let top_limb = a.add(n - 1).read();
			let top_bits = (top_limb | Limb(1)).bit_width_nonzero();
			n.unchecked_mul(Limb::BITS).unchecked_add(top_bits).unchecked_sub(Limb::BITS)
		}
	}
}
...
pub unsafe fn mul_1_unchecked(
	...
	unsafe {
		...
		let mut carry = c;
		while rp != re {
			let [lo, hi] = Limb::mul(ap.read(), b, carry, Limb(0));
			rp.write(lo);
			carry = hi;
...
pub unsafe fn mul_1_unchecked_(rp: *mut Limb, re: *mut Limb, b: Limb, c: Limb) -> Limb {
	unsafe {
		...
		let mut carry = c;
		while rp != re {
			let [lo, hi] = Limb::mul(rp.read(), b, carry, Limb(0));
			rp.write(lo);
			carry = hi;
